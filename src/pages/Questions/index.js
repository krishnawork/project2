...
componentDidMount = () => {
    ...
    if (type==='emotional') {
      ...
      this.state.questions = [
        ...
        this.state.questions = [
            {
              question: 'Draw-A-Person- I want you to draw a person as well as you can. After the completion of the first drawing, the participant is instructed to, Now draw a [opposite sex of the first picture] as well as you can. (Only in case of pencil drawing not in the colour drawing).',
              options: [{value: 'A', option: ''}],
              selected: 'A'
            },
            {
              question: 'Draw-A-House- Here I want you to draw a tree as well as you can.',
              options: [{value: 'A', option: ''}],
              selected: 'A'
            },
            {
                ...
                <div style={{background: '#ECF3F8'}}>
                    <div style={isMobile?mobStyle:webStyle}>
                        ...
                        <div style={{background: '#F6F6F6', padding: '30px 20px', fontFamily: 'Nunito-Bold'}}>
                            <div>Mindlyf Emotional wellness {this.props.location.state && this.props.location.state === "paid-test" ? "PAID-TESTS" :"SELF-TESTS"}</div>
                            <div style={{fontFamily: 'Roboto'}}>Question {this.state.number} of {this.state.questions.length}</div>
                        </div>
                            { localStorage.getItem("type") != "HTP" && localStorage.getItem("type") != "CAT" && localStorage.getItem("type") != "ROR" && localStorage.getItem("type") != "TAT" ?
                            <CardBody>
                            ...
                            <CardBody>
                              <div style={{fontSize: '14px'}}>
                                <div style={{fontFamily: 'Nunito-Bold', fontSize: '18px'}}>This test is a measure of personality. Now please be seated comfortably as now I am going to present you with some papers where you have to draw a house, tree, and a person respectively.</div><br></br>
                                <div style={{fontFamily: 'Nunito-Bold', fontSize: '16px'}}>{this.state.questions[this.state.number-1].question}</div>
                                <input type="file" className="form-control" value={this.state.image_htp} onChange={(event) => this.onChange(event)}/>
                                {this.state.image_url != null ?
                                  <img className="question-image" width={350} height={350} src={this.state.image_url} />
                                  : ''
                                }
                                <div style={{margin: '20px 0px'}}><Button onClick={this.next} style={{borderRadius: '100px', background: '#fff', border: 'solid thin #DF8F06', padding: '10px 20px', fontSize: '14px', color: '#DF8F06'}}>Continue<FontAwesomeIcon icon={faChevronRight} style={{color: '#DF8F06', marginLeft: '20px'}}/></Button></div>
                              </div>
